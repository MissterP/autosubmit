...
Autosubmit can send an email when one or more
:doc:`CPMIP performance metrics </userguide/performance/index>`
fall outside a target range. The feature reuses the same ``MAIL.NOTIFICATIONS``
and ``MAIL.TO`` settings as job-status notifications, so make sure those are
enabled first (see the previous section).

CPMIP metrics are designed for the **simulation job** (commonly named
``SIM``), which runs per chunk and exposes the calendar metadata (``CHUNK``,
``CHUNKSIZE``, ``CHUNKSIZEUNIT``) required to compute SYPD and CHSY. Declare
the ``CPMIP_THRESHOLDS`` block under that job in ``jobs_<EXPID>.yml``. Each
entry maps a metric name (``SYPD``, ``CHSY``, ``CORE_HOURS``) to its target
shape:
...
* ``THRESHOLD``: numeric target the metric is compared against. Must be > 0.
* ``COMPARISON``: ``greater_than`` (metric must be ≥ threshold) or
  ``less_than`` (metric must be ≤ threshold).
* ``%_ACCEPTED_ERROR``: percentage slack applied to the threshold before
  flagging a violation. For ``greater_than`` the effective bound is
  ``THRESHOLD * (1 - error/100)``; for ``less_than``,
  ``THRESHOLD * (1 + error/100)``. Use ``0`` for a strict check.

When the job completes, Autosubmit recomputes the configured metrics, applies
the bound, and — if any metric is outside it — sends a single
...
With the configuration above, suppose the simulation completes with
``SYPD = 3.9``, ``CHSY = 55000``, and ``CORE_HOURS = 1280``. All three fall
outside their effective bounds (SYPD ``≥ 4.5`` after the 10% slack;
CHSY ``≤ 52500`` after the 5% slack; CORE_HOURS ``≤ 1050`` after the 5%
slack), so Autosubmit sends a single email looking like this:
...
Metrics that pass their thresholds are omitted from the body, and no email is
sent at all when every configured metric is within its effective bound.
